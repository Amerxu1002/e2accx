...
type fakeNotifyEnqueuer struct{ calls int }
...
func TestHoldForApprovalCoreSkipsNotificationEnqueueWhenSuppressed(t *testing.T) {
	pool := testutil.TestDB(t)
	store := identity.NewStore(pool)
	smtpRelay := outbound.NewSMTPRelay(&config.OutboundSMTPConfig{})
	sender := outbound.NewSender(smtpRelay, "test.e2a.dev")
	...
	user, err := store.CreateOrGetUser(ctx, "owner@example.com", "Owner", "google-suppressed")
	if err != nil {
		t.Fatalf("CreateOrGetUser: %v", err)
	}
	if _, err := store.ClaimOrCreateDomain(ctx, "example.com", user.ID); err != nil {
		t.Fatalf("ClaimOrCreateDomain: %v", err)
	}
	agent, err := store.CreateAgent(ctx, "agent@example.com", "example.com", "", "", "", user.ID)
	if err != nil {
		t.Fatalf("CreateAgent: %v", err)
	}
	agent.SuppressNotifications = true

	enq := &fakeNotifyEnqueuer{}
	api.SetNotifyEnqueuer(enq)

	msg, err := api.HoldForApprovalCore(ctx, agent, outbound.SendRequest{To: []string{"to@example.com"}, Subject: "hello", Body: "world"}, "send", "")
	if err != nil {
		t.Fatalf("HoldForApprovalCore: %v", err)
	}
	if enq.calls != 0 {
		t.Fatalf("notify enqueuer calls = %d, want 0", enq.calls)
	}
	var status string
	var notifyJobID *int64
	if err := pool.QueryRow(ctx, `SELECT status, notify_job_id FROM messages WHERE id=$1`, msg.ID).Scan(&status, &notifyJobID); err != nil {
		t.Fatalf("read message row: %v", err)
	}
	if status != identity.MessageStatusPendingReview {
		t.Fatalf("message status = %q, want %q", status, identity.MessageStatusPendingReview)
	}
	if notifyJobID != nil {
		t.Fatalf("notify_job_id = %v, want nil", *notifyJobID)
	}
}
